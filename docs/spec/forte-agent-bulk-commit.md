# `forte agent --bulk-commit` Spec

> **File choice:** these scenarios live in a sibling file, `docs/spec/forte-agent-bulk-commit.md`, rather than appended to `docs/spec/forte-agent.md`, to keep the base agent spec focused on the default one-at-a-time review flow. This file assumes familiarity with `docs/spec/forte-agent.md` (vault discovery, the stubbed-LLM convention, the pipeline stages) and only specifies what `--bulk-commit` changes.

Behavior spec for the `--bulk-commit` flag on `forte agent process` and `forte agent ingest`. Instead of walking the user through each proposed change one at a time across two separate review points (entities/links, then fields — see `docs/spec/forte-agent.md`), `--bulk-commit` collapses **both** review points into a **single editor session**: every proposed new entity, every proposed link, and every proposed field update (eagerly extracted for all proposed entities, not just ones the user has approved yet) is written into one text file, organized into three sections, each line pre-filled with a `[y]` action. The file is handed to an editor; whatever comes back is parsed into approve/reject decisions and committed in one pass.
...
### Scenario: All proposals are presented in one editor pass, unlike the default flow's two review points
...
Then exactly one call is made to the (stubbed) editor boundary, unlike the default flow's separate entity-review and field-review prompts
...
When the user runs `forte agent process 7 --bulk-commit --yes`
Then the stubbed editor boundary is never invoked
And every proposal (the new entity and its field update) is auto-approved and committed, exactly as `--yes` behaves without `--bulk-commit`
...
### Scenario: `--bulk-commit --dry-run` together — editor runs, nothing is committed
...
When the user runs `forte agent process 7 --bulk-commit --dry-run`
...
### Scenario: `agent ingest --bulk-commit` end-to-end
...
And the LLM is stubbed to extract a candidate entity, resolve it as new, and extract a field value for it
And the stubbed editor returns the file contents unchanged (all `[y]`)
When the user runs `forte agent ingest kickoff.md --bulk-commit`
...
And the same bulk-commit flow described above then runs against that document id, invoking the stubbed editor exactly once
And the process exits with status code 0
...
- **Everything already out of scope for the base agent flow** — see `docs/spec/forte-agent.md`'s `## Out of scope` section (`ingest_changes` persistence/resume, embeddings/vector candidate discovery, per-vault prompt overrides, per-step model overrides, field-value provenance, batch/folder processing, live-model evaluation). All of that applies unchanged to `--bulk-commit`.
