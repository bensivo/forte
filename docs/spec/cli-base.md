...
And the help text lists the `vault` subcommand
...
### Scenario: `vault` is a registered subcommand group

```gherkin
Given the `forte` CLI is installed and available on PATH
When the user runs `forte --help`
Then the help text lists `vault` in the Commands section
```
...
- Behavior of individual subcommands — covered in their own specs (e.g. the `forte vault` spec).
