...
:class:`~forte.model.llm.Usage` of the call, so the orchestrator can
accumulate usage across the run.

The steps compose the prompt templates + parse functions in
:mod:`forte.service.agent._prompts` with the reusable retry helper in
:mod:`forte.service.agent._structured`. Semantic, non-error filtering that must NOT
...
from forte.service.entity_service import find_candidates
...
def extract_entities(
    llm: ILlmClient, *, doc_text: str, schema_names: list[str]
