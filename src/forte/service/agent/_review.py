...
review point in the pipeline hands a batch of :class:`~forte.model.
agent.ProposedChange` objects to a :class:`Reviewer` and gets back
one :class:`~forte.model.agent.Decision` per change, in order.
...
The concrete interactive Rich TUI reviewer used by the CLI lives in the
controller layer and is NOT implemented here.
...
from forte.model.agent import Decision, ProposedChange
