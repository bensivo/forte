...
This is the ONE place in the new stack where interactive prompting for
proposed changes happens. It implements the `Reviewer` protocol
(`forte.service.agent_service.Reviewer`) so the pipeline itself never
imports `click` -- see `forte/service/agent/_review.py` for the seam this
fills.
...
This class is NOT a Click group -- it is plain, user-interface code, and the
style guide has no other slot for a non-Click interactive component, so it
lives in `controller/` alongside the Click groups. `CliAgentController`
constructs it directly and passes it into `AgentService.process_document` as
the injected `Reviewer`.
"""
