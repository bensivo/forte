"""In-memory data models for the agent pipeline, plus its typed errors.
...
storage access — so they are trivially serializable and a future web layer
...
class RunState:
    ...
    def add_usage(self, u: Usage) -> None:
        ...
        Args:
            u (Usage): The usage to add to the running total.
...
class CommitReport:
    ...
    def successes(self) -> list[CommitResult]:
        """The results of every change that committed successfully."""
        return [r for r in self.results if r.success]

    @property
    def failures(self) -> list[CommitResult]:
        """The results of every change that failed to commit."""
        return [r for r in self.results if not r.success]


@dataclass
class ProcessResult:
    """The outcome of one agent run over a document.
...
class StructuredCallError(AgentError):
    """Raised when a structured LLM call fails every retry; carries the last error."""
